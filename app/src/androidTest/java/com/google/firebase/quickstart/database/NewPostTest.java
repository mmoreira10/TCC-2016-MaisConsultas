...
public class NewPostTest {
    ...
    public void newPostTest() {
        ...
        ViewInteraction appCompatButton = onView(
                allOf(withId(R.id.btnAcessar), withText(R.string.sign_up),
        ...
        ViewInteraction floatingActionButton = onView(
                allOf(withId(R.id.btnNovaConsulta), isDisplayed()));
        floatingActionButton.perform(click());

        // Enter post title
        ViewInteraction appCompatEditText4 = onView(
                allOf(withId(R.id.edtEspecialidade), isDisplayed()));
        appCompatEditText4.perform(replaceText(postTitle));

        // Enter post content
        ViewInteraction appCompatEditText5 = onView(
                allOf(withId(R.id.edtMedico), isDisplayed()));
        appCompatEditText5.perform(replaceText(postContent));

        // Click submit button
        ViewInteraction floatingActionButton2 = onView(
                allOf(withId(R.id.btnAddConsulta), isDisplayed()));
        ...
        ViewInteraction textView = onView(
                allOf(withId(R.id.tvEspecialidade), withText(postTitle), isDisplayed()));
        textView.check(matches(withText(postTitle)));

        // Check that the content is correct
        ViewInteraction textView2 = onView(
                allOf(withId(R.id.tvMedico), withText(postContent), isDisplayed()));
